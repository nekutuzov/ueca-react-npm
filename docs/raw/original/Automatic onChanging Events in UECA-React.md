...
- **Rewrite the value by returning it, never by assigning it.** `model.<prop> = newValue` inside `onChanging<Prop>` is a re-entrant assignment into a property that is still being assigned, and the model rejects it with a `Re-entrant assignment to property "<name>"` error. The return value is the channel. Assigning a *different* property from the handler is allowed. Writing the value that is **already** being assigned is not refused: it discards nothing, and it is what a converging binding does when a derived two-way binding pushes its result back through its own setter.