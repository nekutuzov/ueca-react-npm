...
## Arrays Across a Property Binding

An array passed through a property binding is **shared, not copied**: the bound property and its source hold
one array object. So a `push` on either end is a `push` on the same array, and the other end sees it
immediately — the same rule that makes `tasks.push(...)` reactive inside one component, extended across the
binding.
...
props: { tasks: [] },
children: {
    taskList: useTaskList({ tasks: UECA.bind(() => model, "tasks") })
}

// either of these is now visible to both:
model.tasks.push(newTask);            // in the parent
model.taskList.tasks.push(newTask);   // in the child
```

**The source has to be observable for this to hold.** A model property always is, so component-to-component
bindings need nothing extra. A plain object does not — bind to one and the two ends end up on separate
arrays, where replacing the array still propagates but mutating it in place is lost in both directions:

```typescript
const store = { tasks: [] };                  // plain: in-place mutation does NOT cross the binding
const store = UECA.observe({ tasks: [] });    // observable: it does
```

**A read-only binding still shares the array.** Read-only governs the *reference*: replacing the array is
refused, mutating it is not, so a `push` through a one-way binding reaches the source. Return a copy from
the getter — `() => model.tasks.slice()` — when the source must be protected from the other end.

This is the same requirement as the section below, one level up: `UECA.observe()` makes the *items*
reactive, and an observable source makes the *array itself* shared. A binding source that is not a model
property should be wrapped in `UECA.observe()`.
