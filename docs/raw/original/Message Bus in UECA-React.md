...
- **model.bus.unicast(messageType, payload)**
  Posts a message to the bus with the specified `messageType` and optional `payload`. This method sends a message to be handled by one subscriber, without the caller needing to know which one. **Exactly one subscriber is expected**: if several are listening for the same message, `unicast` throws before delivering anything — use `broadcast` when more than one should receive it. If nobody is subscribed, it resolves with `undefined`.
  - **Parameters**:  
...
- **model.bus.castTo(modelId, messageType, payload)**  
  Posts a message to the bus for the subscriber matching `modelId` with the specified `messageType` and optional `payload`. This method sends a message to be handled by one subscriber whose `model.fullId()` is `modelId`. **Exactly one subscriber is expected here too**: two models answering to the same id means their identities collide, so `castTo` throws before delivering anything rather than picking one.
  - **Parameters**:
...
> **The one-subscriber contract belongs to `castTo` and `unicast` only, and it is checked before anything is
> dispatched** — no handler runs, so no side effect lands, and the error reaches the call site that chose the
> wrong method. `broadcast` is never in violation, however many subscribers answer. The two are not
> interchangeable: `castTo` needs the recipient's `fullId`, while `unicast` exists precisely for when the
> caller does not know it.

> **A message that declares no payload takes no argument.** `unicast("GetCurrentTime")` is how it is called;
> passing an explicit `undefined` placeholder is a compile error. Earlier code that wrote
> `unicast("GetCurrentTime", undefined)` must drop the second argument.
